...
sum_year_long$grouping <- ifelse(sum_year_long$year < 2004, 1, 2)
...
season_na <- tibble(season, avg_temp, min_temp, max_temp)
...
ggsave("Figures/boxplot_season.png", plot = temp_boxplot_gap, 
...
(temp_year_avg <- ggplot(sum_year_long, aes(x = year, y = temp, 
...
(temp_year_model <- ggplot(pred.mm) + 
                      ...
                      geom_line(data = sum_year_long, aes(x = year, y = temp, 
                                                          color = type, 
                                                          group = interaction(type, grouping))) +
                      geom_hline(yintercept = 0, linetype = "dashed") +
                      ylab(label = "Temperature (˚C)") +
                      xlab(label = "Year") +
                      climate_theme +
